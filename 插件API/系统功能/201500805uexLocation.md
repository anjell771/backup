...
# 3、更新历史
 API 版本：uexLocation-3.0.19(iOS) uexLocation-3.0.6（Android）
 最近更新时间：2015-06-19
...
| 3.0.6  | 支持iOS8.0定位  | sdk升级  |
| 3.0.5  | 支持iOS6.0以下版本  | 修复getAddress方法flag为1时返回json格式不 正确的问题  |
| 3.0.4 |  添加json格式地址的参数 | 移动插件包中的定位jar包和so文件至引擎中  |
| 3.0.3  | 重写uexLocation定位功能  | 新增getAddress接口参数  |
| 3.0.2  | 重写uexLocation定位功能  | 修改uexLocation不能在关闭gprs 和wifi的 时候使用gps 获取坐标  |
| 3.0.1  | 修复经纬度反向解析地址时，无法获取地址的BUG  | 修改配置文件兼容IDE|
| 3.0.0  | 获取当前定位和地址功能插件  | 定位功能插件|
